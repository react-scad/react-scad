...
async function scaffold(opts: {
	...
	await new Promise<void>((resolvePromise, reject) => {
		const child = spawn("git", ["clone", "--depth", "1", EXAMPLE_REPO, targetDir], {
			stdio: "inherit",
		});
		child.on("close", (code) =>
			code === 0 ? resolvePromise() : reject(new Error(`git clone exited with ${code}`)),
		);
	});
	...
	const updatedPkg = pkg.replace(/"name":\s*"[^"]*"/, `"name": "${packageName}"`);
	...
	if (noInstall) {
		...
		await new Promise<void>((resolvePromise, reject) => {
			...
			child.on("close", (code) =>
				code === 0 ? resolvePromise() : reject(new Error(`npm install exited with ${code}`)),
			);
		});
		log.blank();
...
const isNewCommand =
	...
	first === "-v" ||
	first === "--version";
const argSource = isNewCommand ? process.argv : [process.argv[0], process.argv[1], "new", ...argv];
