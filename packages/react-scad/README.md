...
- Use [@react-scad/cli](../cli) to bundle and run `.jsx`/`.tsx` entry files
...
To build and run entry files from the command line, install the CLI as well:

```bash
npm install @react-scad/cli
```
