...
function toVec2(v: unknown): Vec2 {
	...
	if (Array.isArray(v) && v.length >= 2) return [toScadExpr(v[0]), toScadExpr(v[1])];
	return 1;
}
...
function toPoints3D(v: unknown): [number, number, number][] {
	...
	return v.map((p) => {
		if (!Array.isArray(p) || p.length < 3) return [0, 0, 0] as [number, number, number];
	...
	switch (type) {
		...
		case "text":
			return {
				...
				halign: props.halign != null ? (props.halign as "left" | "center" | "right") : undefined,
				valign:
					props.valign != null
						? (props.valign as "top" | "center" | "baseline" | "bottom")
						: undefined,
				direction:
					props.direction != null ? (props.direction as "ltr" | "rtl" | "ttb" | "btt") : undefined,
...
export function updateScadNode(node: ScadNode, nextProps: Record<string, unknown>): void {
	switch (node.kind) {
		...
		case "polyhedron":
			...
			if (nextProps.convexity != null) node.convexity = num(nextProps.convexity, 1);
			break;
		case "square": {
			const size = nextProps.size ?? node.size;
		...
		case "linear_extrude":
			...
			if (nextProps.center !== undefined) node.center = nextProps.center === true;
		...
		case "text":
			...
			if (nextProps.halign != null) node.halign = nextProps.halign as "left" | "center" | "right";
			if (nextProps.valign != null)
				node.valign = nextProps.valign as "top" | "center" | "baseline" | "bottom";
			if (nextProps.direction != null)
				node.direction = nextProps.direction as "ltr" | "rtl" | "ttb" | "btt";
			if (nextProps.language != null) node.language = String(nextProps.language);
		...
		case "surface":
			...
			if (nextProps.center !== undefined) node.center = nextProps.center === true;
			if (nextProps.invert !== undefined) node.invert = nextProps.invert === true;
			if (nextProps.convexity != null) node.convexity = num(nextProps.convexity, 1);
			break;
		case "import":
			if (nextProps.file != null) node.file = String(nextProps.file);
